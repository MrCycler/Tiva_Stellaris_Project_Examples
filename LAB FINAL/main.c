...
void main(void){
	...
	config_UART1();
	/*Inicializar valores*/
	uint8_t Entrada[] = "Introduzca la velocidad de la bomba en porcentaje:\n\r";
	int valor;
	uint32_t Freq=500;//Frecuencia inicial 500 Hz
	uint8_t arreglo[10];
	// Se genera la señal PWM con Frecuencia 5KHz, DC=50%
	PWM_OUT (Freq,0);

	while(1){
		/*Identifica carácter, I y M para variar frecuencia, L y J para DC */
		//Función de transmisión de byte.....
		...
		if ((valor<=100)&&(valor>=50))
			...
			Freq=8*valor-300;
			PWM_OUT(Freq,50);
			txmens_uart0("Velocidad de la bomba establecida en ");
			txmens_uart0(arreglo);
			txmens_uart0("por ciento \n\r");
			}
		else if(valor==0)
			{
			PWM_OUT(Freq,0);
			txmens_uart0("Bomba apagada \n\r");
			}
			else
			{
			txmens_uart0("Comando invalido \n\r");
			}
		}
}
