...
void main(void)
{
	...
	config_leds();
		...
		while(centM==0)
		{
			...
			{
				if(cent1==0){number_temp = number_temp + 2; cent1=1;}else{number_temp = number_temp - 2; cent1=0;}
				do {
			...
			{
				if(cent2==0){number_temp = number_temp + 1; cent2=1;}else{number_temp = number_temp - 1; cent2=0;}
				do {
					temp=GPIO_PORTA_DATA_R; // espera que sea liberado (soltado)
			...
			{
				suma = number_temp + suma;
				centM=1;
				do {
					temp=GPIO_PORTA_DATA_R; // espera que sea liberado (soltado)
				   } while((temp&0x20)==0x20);
			}
		}

		while(suma>0){
			GPIO_PORTF_DATA_R &= ~(0X0E);
			GPIO_PORTF_DATA_R |=suma*2;
			for(n = 0; n < 900000; n++);
			suma--;
		}
